...
export default function MyStoresPage() {
    const nav = useNavigate(), [q, setQ] = useState(""), [filter, setFilter] = useState("all"); const rows = useMemo(() => data.filter(x => x[0].toLowerCase().includes(q.toLowerCase()) && (filter === "all" || x[4] === filter)), [q, filter]); return <main className="ms-page">
